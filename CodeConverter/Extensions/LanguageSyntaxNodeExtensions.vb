...
Public Module LanguageSyntaxNodeExtensions
    ...
    ''' <returns></returns>
    <Extension>
    Public Function DoConversion(sourceTree As CS.CSharpSyntaxNode, pSemanticModel As SemanticModel, defaultVbOptions As DefaultVbOptions, skipAutoGenerated As Boolean, isProjectConversion As Boolean, reportException As Action(Of Exception), progress As IProgress(Of ProgressReport), globalSymbols As List(Of Dictionary(Of String, String)), cancelToken As CancellationToken) As VB.VisualBasicSyntaxNode
        IgnoredIfDepth = 0
        Dim visualBasicSyntaxNode1 As VB.VisualBasicSyntaxNode
        SyncLock _thisLock
            ClearMarker()
            visualBasicSyntaxNode1 = sourceTree?.Accept(New NodesVisitor(New ConvertRequest(skipAutoGenerated, isProjectConversion, progress, cancelToken), pSemanticModel, globalSymbols, defaultVbOptions, reportException))
        End SyncLock
