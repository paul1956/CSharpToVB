...
Public Module LanguageSyntaxNodeExtensions
    ...
    ''' <returns></returns>
    <Extension>
    Public Function DoConversion(sourceTree As CS.CSharpSyntaxNode, pSemanticModel As SemanticModel, defaultVbOptions As DefaultVbOptions, skipAutoGenerated As Boolean, isProjectConversion As Boolean, reportException As Action(Of Exception), progress As IProgress(Of ProgressReport), globalSymbols As List(Of String), cancelToken As CancellationToken) As VB.VisualBasicSyntaxNode
        IgnoredIfDepth = 0
        Dim visualBasicSyntaxNode1 As VB.VisualBasicSyntaxNode
        SyncLock _thisLock
            ClearMarker()
            visualBasicSyntaxNode1 = sourceTree?.Accept(New NodesVisitor(globalSymbols, New ConvertRequest(skipAutoGenerated, isProjectConversion, progress, cancelToken), pSemanticModel, defaultVbOptions, reportException))
        End SyncLock
        Return visualBasicSyntaxNode1
    End Function

    Public Function GetGlobalSymbols(compilation As Compilation) As List(Of String)
        Dim applicationSymbols As New List(Of String)
        Dim externalSymbols As New List(Of String)

        GetNamespaceSymbols(compilation.GlobalNamespace.GetTypeMembers(), compilation.Assembly, externalSymbols, applicationSymbols)
        For Each node As INamespaceOrTypeSymbol In compilation.GlobalNamespace.GetMembers()
            Dim namespaceSymbol As INamespaceSymbol = TryCast(node, INamespaceSymbol)
            If namespaceSymbol IsNot Nothing Then
                GetNamespaceSymbols(namespaceSymbol.GetMembers(), compilation.Assembly, externalSymbols, applicationSymbols)
                GetNamespaceSymbols(namespaceSymbol.GetTypeMembers(), compilation.Assembly, externalSymbols, applicationSymbols)
            End If
        Next
        Dim globalSymbols As New List(Of String)
        For Each e As String In externalSymbols
            Dim s As String = e.Split("."c)(0)
            If applicationSymbols.Contains(s, compareTypeOnly:=True, StringComparison.OrdinalIgnoreCase) Then
                globalSymbols.Add(s)
            End If
        Next
        ' TODO workaround for not being able to be application symbols
        globalSymbols = New List(Of String) From {
            "Path",
            "Rect"
            }
    ...
    Private Sub GetNamespaceSymbols(namespaceSymbols As IEnumerable(Of INamespaceOrTypeSymbol), assembly As IAssemblySymbol, externalSymbols As List(Of String), applicationSymbols As List(Of String))
        If namespaceSymbols Is Nothing Then Exit Sub
        For Each node As INamespaceOrTypeSymbol In namespaceSymbols
            Dim namespaceSymbol As INamespaceSymbol = TryCast(node, INamespaceSymbol)
            If namespaceSymbol IsNot Nothing Then
                GetNamespaceSymbols(namespaceSymbol.GetMembers(), assembly, externalSymbols, applicationSymbols)
                GetNamespaceSymbols(namespaceSymbol.GetTypeMembers(), assembly, externalSymbols, applicationSymbols)
            Else
                Dim typeSymbol As ITypeSymbol = TryCast(node, ITypeSymbol)
                If typeSymbol Is Nothing Then Continue For
                If typeSymbol.ContainingAssembly.Equals(assembly, SymbolEqualityComparer.Default) Then
                    If Not applicationSymbols.Contains(typeSymbol.ToString(), StringComparer.Ordinal) Then
                        applicationSymbols.Add(typeSymbol.ToString())
                    End If
                Else
                    If Not externalSymbols.Contains(typeSymbol.ToString(), StringComparer.Ordinal) Then
                        externalSymbols.Add(typeSymbol.ToString())
                    End If
                End If
            End If
        Next node
