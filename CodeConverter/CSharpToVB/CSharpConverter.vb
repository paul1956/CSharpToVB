...
Namespace CSharpToVBCodeConverter.DestVisualBasic
    ...
    Partial Public Class CSharpConverter
        ...
        ''' <returns></returns>
        Public Shared Function Convert(SourceTree As CS.CSharpSyntaxNode, SkipAutoGenerated As Boolean, pSemanticModel As SemanticModel, ProgressReport As TextProgressBar, CancelToken As CancellationToken) As VB.VisualBasicSyntaxNode
            Dim visualBasicSyntaxNode1 As VB.VisualBasicSyntaxNode
            s_originalRequest = New ConvertRequest(SkipAutoGenerated, ProgressReport, CancelToken)
            SyncLock s_thisLock
                ClearMarker()
                s_usedStacks.Push(s_usedIdentifiers)
                s_usedIdentifiers.Clear()
                visualBasicSyntaxNode1 = SourceTree?.Accept(New NodesVisitor(pSemanticModel))
