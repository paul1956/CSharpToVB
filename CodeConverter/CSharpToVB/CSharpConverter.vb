...
Namespace IVisualBasicCode.CodeConverter.Visual_Basic
    ...
    Partial Public Class CSharpConverter
        ...
        Private Shared ReadOnly UsedIdentifierStack As New Stack(New Dictionary(Of String, SymbolTableEntry))
        ...
        Public Shared Function Convert(SourceTree As CS.CSharpSyntaxNode, _SemanticModel As SemanticModel) As VB.VisualBasicSyntaxNode
            Dim visualBasicSyntaxNode1 As VB.VisualBasicSyntaxNode
            If OriginalRequest Is Nothing Then
                OriginalRequest = New ConvertRequest("cs2vb", Sub() Return, _ProgressBar:=Nothing)
            End If
            SyncLock ThisLock
                ClearMarker()
        ...
        Public Shared Function ConvertText(SourceText As String, References As MetadataReference()) As ConversionResult
            UsedIdentifierStack.Push(UsedIdentifiers)
            UsedIdentifiers.Clear()
            ...
            Dim CSharpParseOption As CS.CSharpParseOptions = New CS.CSharpParseOptions(
                ...
                PreprocessorSymbols)
            Dim tree As SyntaxTree = CS.SyntaxFactory.ParseSyntaxTree(text:=Text.SourceText.From(SourceText),
            ...
            Try
                Dim ConvertedNode As VB.VisualBasicSyntaxNode = Convert(
                        SourceTree:=DirectCast(tree.GetRoot(), CS.CSharpSyntaxNode),
