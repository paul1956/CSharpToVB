...
Friend Module ProcessFileUtilities
    ...
    Private Async Function Convert_Compile_ColorizeAsync(
        ...
        optionalReferences() As MetadataReference) As Task(Of Boolean)
    ...
    Friend Async Function ConvertSnippetOfTopLevelStmt(mainForm As Form1, sourceCode As String) As Task
        SetButtonStopCursorAndCancelToken(mainForm:=mainForm, stopButtonVisible:=True)
        ...
        mainForm._requestToConvert = New ConvertRequest(My.Settings.SkipAutoGenerated, isProjectConverstion:=False, New Progress(Of ProgressReport)(AddressOf mainForm.StatusStripConversionProgressBar.Update), mainForm._cancellationTokenSource.Token) With
                {
    ...
    Friend Async Function ProcessFileAsync(mainForm As Form1, sourceFileNameWithPath As String, targetDirectory As String, sourceLanguageExtension As String, csPreprocessorSymbols As List(Of String), vbPreprocessorSymbols As List(Of KeyValuePair(Of String, Object)), optionalReferences() As MetadataReference, skipAutoGenerated As Boolean, isProjectConversion As Boolean, cancelToken As CancellationToken) As Task(Of Boolean)
        ...
        With mainForm
            ...
            If lines > 0 Then
                ...
                ._requestToConvert = New ConvertRequest(skipAutoGenerated, isProjectConversion, New Progress(Of ProgressReport)(AddressOf mainForm.StatusStripConversionProgressBar.Update), cancelToken) With {
                        .SourceCode = mainForm.ConversionInput.Text
                    }
                If Not Await Convert_Compile_ColorizeAsync(mainForm, ._requestToConvert, csPreprocessorSymbols, vbPreprocessorSymbols, optionalReferences).ConfigureAwait(True) Then
